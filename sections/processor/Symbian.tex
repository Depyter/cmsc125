...
\end{minted}
\caption{Priority bitmap and ready-queue anchors in \texttt{nklib.h}.}
...
\end{minted}
\caption{Core scheduler state in \texttt{nk\_priv.h}.}
...
/* ... background and other system-server bands omitted ... */
...
TInt DThread::CalcDefaultThreadPriority()
	...
	if (tp>=0)
		...
		if ((r > KMaxPriorityWithoutProtServ) &&
			!(this->HasCapability(ECapabilityProtServ, /* diagnostic omitted */)))
			r = KMaxPriorityWithoutProtServ;
		}
	return r;
	}
\end{minted}
\caption{Process-priority bands and thread-priority mapping. Files:
\texttt{kern\_priv.h} and \texttt{sthread.cpp}.}
...
\end{minted}
\caption{Timeslice state and round-robin rotation. Files:
\texttt{nk\_priv.h}, \texttt{kern\_priv.h}, and \texttt{nkern/sched.cpp}.}
...
\end{minted}
\caption{Process-priority propagation in \texttt{sprocess.cpp}.}
...
\end{minted}
\caption{Executive-side thread setup. File: \texttt{sthread.cpp}.}
...
TInt NThread::Create(SNThreadCreateInfo& aInfo, TBool aInitial)
	...
	if (!aInitial)
		...
		/* copy parameter block to the new stack, then build the entry frame */
...
\end{minted}
\caption{Nanokernel thread bootstrap frame construction. File:
\texttt{arm/ncthrd.cpp}.}
...
__NAKED__ void TScheduler::Reschedule()
	...
	/* choose the highest-priority ready thread */
	/* restore the next thread and return */
	}
\end{minted}
\caption{Hot reschedule path in \texttt{arm/ncsched.cia}.}
...
Files: \texttt{nkernsmp/sched.cpp}, \texttt{nk\_bal.cpp}, and
\texttt{sthread.cpp}.}
\label{lst:sym-smp}
\end{listing}
...
\end{minted}
\caption{Priority inheritance through cleanup-linked propagation. File:
\texttt{sthread.cpp}.}
